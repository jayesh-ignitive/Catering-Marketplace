...
import {
  WorkspacePremiumModulePreview,
  type PreviewKind,
} from "./WorkspacePremiumModulePreview";
...
function PremiumPopupCard({
  title,
  description,
  icon: Icon,
}: Pick<WorkspaceModulePlaceholderProps, "title" | "description" | "icon">) {
  ...
  return (
    <div
      role="dialog"
      aria-modal="true"
      aria-labelledby="ws-premium-module-title"
      className="admin-panel-card mx-auto w-full max-w-2xl overflow-hidden p-8 text-center shadow-[0_25px_50px_-12px_rgba(0,0,0,0.18)] md:p-12"
    >
      ...
      <h2 id="ws-premium-module-title" className="font-heading mt-4 text-xl font-bold text-brand-text-dark">
        {title}
      </h2>
      <p className="mt-3 text-sm leading-relaxed text-brand-text-muted">{description}</p>
      <p className="mt-4 text-sm font-semibold text-brand-text-dark">{ws.modules.premiumIncluded}</p>
...
export function WorkspaceModulePlaceholder({
  ...
  if (variant === "premium") {
    if (preview) {
      return (
        <div className="relative mx-auto w-full min-h-[min(78vh,720px)] min-w-0 max-w-5xl overflow-hidden rounded-3xl">
          <div
            aria-hidden
            className="pointer-events-none select-none scale-[1.015] blur-[7px] saturate-[0.85] brightness-[0.97]"
          >
            <WorkspacePremiumModulePreview kind={preview} />
          </div>
          <div
            aria-hidden
