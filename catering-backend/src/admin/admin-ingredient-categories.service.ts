...
import {
  ...
  CreateIngredientCategoryTranslationDto,
} from './dto/create-ingredient-category.dto';
...
function toTranslationItem(
  row: IngredientCategoryTranslation,
): AdminIngredientCategoryTranslationItem {
  return {
    id: row.id,
...
function toCategoryItem(row: IngredientCategory): AdminIngredientCategoryItem {
  return {
    id: row.id,
...
@Injectable()
export class AdminIngredientCategoriesService {
  constructor(
    @InjectRepository(IngredientCategory)
    private readonly categories: Repository<IngredientCategory>,
    @InjectRepository(IngredientCategoryTranslation)
    private readonly translations: Repository<IngredientCategoryTranslation>,
    @InjectRepository(Language)
    private readonly languages: Repository<Language>,
  ) {}

  async list(): Promise<AdminIngredientCategoryItem[]> {
  ...
  async create(
    ...
    const baseTranslations: CreateIngredientCategoryTranslationDto[] = [
      {
  ...
  async upsertTranslation(
    categoryId: string,
    dto: UpsertIngredientCategoryTranslationDto,
  ): Promise<AdminIngredientCategoryItem> {
  ...
  private async findOneOrThrow(id: string): Promise<AdminIngredientCategoryItem> {
  ...
  private assertTwoLevelConstraint(parent: IngredientCategory | null): void {
    if (parent?.parent?.id) {
      throw new BadRequestException(
