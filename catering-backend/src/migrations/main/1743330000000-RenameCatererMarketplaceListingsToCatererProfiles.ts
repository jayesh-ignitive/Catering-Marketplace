 ...
 */
export class RenameCatererMarketplaceListingsToCatererProfiles1743330000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    if (
      names.has('IDX_cml_published_city') &&
      !names.has('IDX_cp_published_city')
    ) {
    ...
    if (
      names.has('IDX_cml_published_category') &&
      !names.has('IDX_cp_published_category')
    ) {
      await queryRunner.query(
    ...
    if (
      names.has('IDX_cml_published_price_band') &&
      !names.has('IDX_cp_published_price_band')
    ) {
      await queryRunner.query(
